	...
	<c:otherwise>
		<c:choose>
			<c:when test="${not empty ad.redirectUrl}">
				<a href="${fn:indexOf(fn:toLowerCase(ad.redirectUrl),'http://')==0?'':ctxPath}${ad.redirectUrl}" <c:if test="${fn:indexOf(fn:toLowerCase(ad.redirectUrl),'http://')==0 }">target="_blank"</c:if>>
