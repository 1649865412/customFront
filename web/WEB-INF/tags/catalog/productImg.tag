...
%>
<c:if test="${empty noLink&&empty isUrlOnly}">
<a base target="_blank" href="javascript:void(0);" <c:if test="${not empty pageScope.alt}"> title="<c:out value="${pageScope.alt}"/>"</c:if>>
</c:if>
