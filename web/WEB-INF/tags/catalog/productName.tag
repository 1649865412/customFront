...
<c:choose>
	<c:when test="${empty interceptLength}">
		<%--<a href="<product:productUrl product="${product}" category="${category}" />" title="<c:out value="${pageScope.productName}"/>"></a>--%>
		${pageScope.productName}
	</c:when>
	<c:otherwise>
		<%--<a href="<product:productUrl product="${product}" category="${category}" />" title="<c:out value="${pageScope.productName}"/>"></a>--%>
		<app:interceptString length="${interceptLength}" str="${pageScope.productName}"></app:interceptString>
	</c:otherwise>
</c:choose>