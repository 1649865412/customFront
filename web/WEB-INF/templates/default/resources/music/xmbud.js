...
function wmpStop(){
 ...
 if(blnStatusBar){window.status=('CBO Player');return true;}
}
...
// 12(MediaOpening) 13(MediaOpen) 20(MediaWaiting) 21(OpeningUnknownURL)
 ...
 if(f==8){capText.innerHTML="CBO Player";}
 if(f==13){
	...
	strMmInfo= "Media (Title) : " + strTitle + " (CBO Player:" + strType +")" + "\n"
...
function evtWmpError(){
 ...
 if(intErrCnt<=3){
	...
	var msg="(CBO  MP:" + intErrCnt +") " +cActTit;
	disp1.innerHTML="CBO Player" +cActTit;
	if(blnStatusBar){window.status=(msg);}
...
function waitMsg(){
 capText.innerHTML="CBO MP Captioning(SMI)";
 if(intExobudStat ==1){disp1.innerHTML=" CBO Player";}
 if(intExobudStat ==2){disp1.innerHTML="CBO Player";}
 if(blnStatusBar){
	if(intExobudStat ==1){window.status=('CBO Player');return true;}
	if(intExobudStat ==2){window.status=('CBO Player'); return true;}
 }
}
...
function vizExobud(){
  window.open("http://www.ccbo.net/","vizExobud");
}
