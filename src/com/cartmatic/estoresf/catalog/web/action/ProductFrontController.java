...
public class ProductFrontController extends GenericStoreFrontController<Product>{
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value="/product/name.html")
	public ModelAndView findProductImages(HttpServletRequest request, HttpServletResponse response)  throws Exception{
		String name = request.getParameter("name");
		List<ProductMedia>productMoreImages=null;
		if(!StringUtil.isEmpty(name)){
			Product product = null;
			product = this.productManager.getProductByNameAndStatus(name, new Short("1"));//根据名称和状态(1为激活状态)查找唯一产品
//			product = product == null ? this.productManager.getById(1) : product;	//id为1的产品设为默认定制产品
			if(product != null){
				//获取产品媒体
				productMoreImages=productMediaManager.findProductMediaByProductIdAndType(product.getProductId(), CatalogConstants.PRODUCT_MEDIA_TYPE_MORE_IMAGE);
				for(ProductMedia pm : productMoreImages){
					pm.set_productId(product.getId());
					pm.setProduct(null);
					pm.setCostPrice(product.getDefaultProductSku().getPrice());
					pm.setShortDesc(product.getProductDescription().getShortDescription());
				}
		...
		JSONArray ja = JSONArray.fromObject(productMoreImages);
	 ...
	 * @return
	 */
	//@RequestMapping(params="doAction=checkOptions", method=RequestMethod.POST)
	@RequestMapping(value="/custom/more.html")
	public ModelAndView checkOptions(HttpServletRequest request, HttpServletResponse response){
		ModelAndView mv = new ModelAndView("/cart/check_options");
		...
		if(cookie == null){
			Customer customer = (Customer) RequestContext.getCurrentUser();
			 sc = shoppingcartManager.initShoppingcart(customer);
		}else{
			sc = this.shoppingcartManager.loadShoppingcartByUuid(cookie.getValue());
			if(sc == null){
				Customer customer = (Customer) RequestContext.getCurrentUser();
				 sc = shoppingcartManager.initShoppingcart(customer);
			}
		}
		shoppingcartManager.refreshCart(sc.getUuid(), request, response);
