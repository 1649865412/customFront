...
public class ShoppingcartController extends BaseStoreFrontController {
	...
	@Autowired
	private PersonalMediaManager personalMediaManager;
    ...
    @RequestMapping(params="doAction=addProductAndOptions", method=RequestMethod.POST)
	public ModelAndView addProductAndOptions(HttpServletRequest request, HttpServletResponse response) throws Exception{
    	...
    	if(cookie!=null){
    		cart = this.shoppingcartManager.loadShoppingcartByUuid(cookie.getValue());
    		if(cart != null){
    			boolean flag = true;
        		for(ShoppingcartItem item : cart.getCartItems()){
        			if(item.getProductSku().getProductSkuCode().equals(product.getDefaultProductSku().getProductSkuCode())){
        				flag = false;
        				break;
        			}
        		}
        		if(flag)
        			cart = shoppingcartManager.addProductToCart(cookie.getValue(), product.getDefaultProductSku().getProductSkuCode(), new Integer(1), null,null, request, response);
    		}else{
    			Customer customer = (Customer) RequestContext.getCurrentUser();
    			Shoppingcart sc = shoppingcartManager.initShoppingcart(customer);
    			cart = shoppingcartManager.addProductToCart(sc.getUuid(), product.getDefaultProductSku().getProductSkuCode(), new Integer(1), null,null, request, response);
    		}
    	...
    	if(cart != null){
    		cart.setMoreOptions(moreOptions);
    		this.shoppingcartManager.save(cart);
    	}
    	
    	String[] personalMediaIds = request.getParameterValues("personalMediaIds");
    	String[] personalMediaTypes = request.getParameterValues("personalMediaTypes");
    	String[] personalMediaDeleteds = request.getParameterValues("personalMedia_deleteds");
    	String[] personaUrls = request.getParameterValues("personalMedia_urls");
    	
    	this.personalMediaManager.savePersonalMedias(cart.getShoppingcartId(), personalMediaIds, personalMediaTypes, personaUrls, personalMediaDeleteds);
 	...
 	public ModelAndView defaultAction(HttpServletRequest request, HttpServletResponse response)  
	{
		...
		Category category = categoryManager.getProductCategoryByCode(Constants.ROOT_CATEGORY_CATALOG, "dayoff78");
		for(ShoppingcartItem item:items){
			...
			if(category != null && !productCategoryManager.isInCategory(item.getProductSku().getProductId(), category.getCategoryPath()+category.getCategoryId()+".")){
...
//		request.setAttribute("shippingRateList",shippingRateList);
	...
	public PersonalMediaManager getPersonalMediaManager() {
		return personalMediaManager;
	}

	public void setPersonalMediaManager(PersonalMediaManager personalMediaManager) {
		this.personalMediaManager = personalMediaManager;
	}
}